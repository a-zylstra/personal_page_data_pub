...
export function countClassifications(artworks: MetObject[]): MediumCount[] {
  const map = new Map<string, number>();

  for (const artwork of artworks) {
    const classification = artwork.department || 'Unknown';

    map.set(classification, (map.get(classification) ?? 0) + 1);
  }
